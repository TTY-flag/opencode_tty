...
description: 漏洞预验证和误报过滤规则。在报告候选漏洞之前使用此 Skill 进行快速过滤，减少误报率。适用于 DataFlow Scanner 和 Security Auditor。
---
...
| 条件 | 检查方法 | 适用场景 |
|------|----------|----------|
| 测试代码 | 文件路径包含 `test/`、`tests/`、`mock/`、`example/`、`_test.c`、`_test.cpp` | 所有扫描 |
| 编译时常量 | 参数为 `sizeof()`、`#define` 常量、`const` 变量、枚举值 | 数据流分析 |
| 相邻边界检查 | ±5 行内存在 `if(len <)`、`if(size >)`、`if(n <=)` 等边界检查 | 数据流分析 |
| 死代码 | 位于 `#if 0`、`#ifdef DEBUG`（非生产）、`if(false)` 块中 | 所有扫描 |
| 安全替代函数 | 已使用 `strncpy`、`snprintf`、`strlcpy` 等安全版本且参数正确 | 数据流分析 |
| 注释代码 | 位于 `/* */` 或 `//` 注释块中 | 所有扫描 |
| 第三方代码 | 文件路径包含 `vendor/`、`third_party/`、`external/`、`deps/` | 所有扫描 |
...
| 非安全随机 | `rand()` 用于负载均衡、UI 随机化、测试数据生成（非密钥/令牌） | 需上下文判断 |
